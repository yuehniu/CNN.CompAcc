...
//              add partial summation data -- 1.3
//              trim -- 1.4
//
// Version    : 1.4
// ---------------------------------------------------
module conv_op#(
      ...
      //input  wire                                               conv_next_ker_valid_at_next_clk, // next set of kernel valid at next clock
        input  wire [(EXPONENT+MANTISSA+1)*K_C*K_H*K_W-1:0]       conv_ker,    // shape: k_c k_h k_w
        input  wire [(EXPONENT+MANTISSA+1)*DATA_H*DATA_W-1:0]     conv_bottom, // shape: data_h data_w
        ...
        input  wire [K_C*(EXPONENT+MANTISSA+1)-1:0]               conv_partial_sum, // partial sum from output buffer (512x14x14)
        output wire [(EXPONENT+MANTISSA+1)*K_C-1:0]               conv_top,    // shape: k_c, no output buffer reg
        ...
        (*mark_debug="TRUE"*)output wire                                               conv_output_valid,
      ...
      //output wire                                               conv_next_partial_sum, // next partial summation data should be valid at next clk
...
//                                          _data1[(EXPONENT+MANTISSA+1)*(K_W-0)-1:(EXPONENT+MANTISSA+1)*(K_W-1)];
    ...
    for(c=0; c<K_C; c=c+1) begin
      for(h=0; h<K_H; h=h+1) begin
        for(w=0; w<K_W; w=w+1)begin
          assign _ker[K_C*K_H*K_W-1-(w+h*K_W+c*K_H*K_W)] = conv_ker[(EXPONENT+MANTISSA+1)*(1+w+h*K_W+c*K_H*K_W)-1 : (EXPONENT+MANTISSA+1)*(w+h*K_W+c*K_H*K_W)];
        end
      end
    end
    for(h=0; h<DATA_H; h=h+1)begin // column-wise
      assign _bottom00[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+15+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*(15+h*DATA_W)];
      assign _bottom01[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+14+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*(14+h*DATA_W)];
      assign _bottom02[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+13+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*(13+h*DATA_W)];
      assign _bottom03[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+12+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*(12+h*DATA_W)];
      assign _bottom04[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+11+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*(11+h*DATA_W)];
      assign _bottom05[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+10+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*(10+h*DATA_W)];
      assign _bottom06[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 9+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 9+h*DATA_W)];
      assign _bottom07[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 8+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 8+h*DATA_W)];
      assign _bottom08[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 7+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 7+h*DATA_W)];
      assign _bottom09[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 6+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 6+h*DATA_W)];
      assign _bottom10[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 5+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 5+h*DATA_W)];
      assign _bottom11[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 4+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 4+h*DATA_W)];
      assign _bottom12[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 3+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 3+h*DATA_W)];
      assign _bottom13[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 2+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 2+h*DATA_W)];
      assign _bottom14[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 1+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 1+h*DATA_W)];
      assign _bottom15[DATA_H-1-h] = conv_bottom[(EXPONENT+MANTISSA+1)*(1+ 0+h*DATA_W)-1 : (EXPONENT+MANTISSA+1)*( 0+h*DATA_W)];
    end
  endgenerate

  reg [(EXPONENT+MANTISSA+1)*K_W-1:0] _pa0_data_0;
  reg [(EXPONENT+MANTISSA+1)*K_W-1:0] _pa1_data_0;
  reg [(EXPONENT+MANTISSA+1)*K_W-1:0] _pa2_data_0;
  reg                                 _pa0_data_valid_0;
  reg                                 _pa1_data_valid_0;
  reg                                 _pa2_data_valid_0;
  reg                                 _conv_partial_sum_valid;
  reg [K_C*(EXPONENT+MANTISSA+1)-1:0] _conv_partial_sum;
  ...
  pe_array3x3#(
    ...
    .MANTISSA(MANTISSA)
      ...
      .pe3_o(conv_top[(EXPONENT+MANTISSA+1)*(K_C-1-0+1)-1:(EXPONENT+MANTISSA+1)*(K_C-1-0)]),
      .pe3_partial_value(_conv_partial_sum[(K_C-1-0+1)*(EXPONENT+MANTISSA+1)-1:(K_C-1-0)*(EXPONENT+MANTISSA+1)]),
      .pe3_next_partial_sum(conv_rd_data_partial_sum), // next partial sum data
      .pe3_valid(conv_output_valid)
    );
  generate
    for(c=1; c<K_C; c=c+1) begin
      pe_array3x3#(
        .EXPONENT(EXPONENT),
        .MANTISSA(MANTISSA)
        ) pe_array(
          ...
          .pe3_array1_valid(_pa1_data_valid_0),
          .pe3_array2_valid(_pa2_data_valid_0),
          .pe3_o(conv_top[(EXPONENT+MANTISSA+1)*(K_C-1-c+1)-1:(EXPONENT+MANTISSA+1)*(K_C-1-c)]),
          .pe3_partial_value(_conv_partial_sum[(K_C-1-c+1)*(EXPONENT+MANTISSA+1)-1:(K_C-1-c)*(EXPONENT+MANTISSA+1)]),
